  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // preserve aspect ratio and fit inside the square cell
        const imageAspect = img.width / img.height;
        let drawW = cellSize;
        let drawH = cellSize;

        if (imageAspect > 1) {
          // wide image -> full width, reduced height
          drawW = cellSize;
          drawH = cellSize / imageAspect;
        } else {
          // tall image -> full height, reduced width
          drawH = cellSize;
          drawW = cellSize * imageAspect;
        }

        const dx = x + (cellSize - drawW) * 0.5;
        const dy = y + (cellSize - drawH) * 0.5;

        // optionally clear the cell first (for transparent background)
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  return (
    <div className="relative w-full h-full dark:bg-black bg-white">
      <canvas
      ...
      {activeItem && (
        ...
        `}
          ...
          ${
            isMoving
              ? "opacity-0 pointer-events-none duration-[100ms] translate-x-[-60%] lg:-translate-y-1/2"
              : "opacity-100 pointer-events-auto duration-[500ms] translate-x-[-90%] lg:-translate-y-1/2"
          }
        `}
          >
